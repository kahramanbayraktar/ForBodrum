...
export function Sidebar({ activeTab, onTabChange, onReportClick, dictionary }: SidebarProps) {
  const menuItems = [
    { id: "map", label: dictionary.menu.map_overview, icon: Map },
    { id: "actions", label: dictionary.menu.civic_actions, icon: Vote },
    { id: "profile", label: dictionary.menu.my_profile, icon: User },
  ]
    ...
    <aside className="hidden md:flex flex-col w-64 h-screen bg-card border-r border-border fixed left-0 top-0 z-50">
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        >
          ...
          {dictionary?.common?.report_issue || "Report Issue"}
        </Button>
